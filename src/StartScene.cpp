...
void StartScene::draw()
{
	...
	TheTextureManager::Instance()->draw("bg", 400, 300,
...
void StartScene::update()
{
	...
	if (CollisionManager::lowerBoundaryCheck(m_pBall))
	{
		m_pBall->setVerticalDirection(-1);
		if (m_pBall->getSpeed() >= 0.1)
			m_pBall->setSpeed(m_pBall->getSpeed() - 1.0f);
	}
	else if (CollisionManager::upperBoundaryCheck(m_pBall))
	{
		m_pBall->setVerticalDirection(1);
		if (m_pBall->getSpeed() >= 0.1)
			m_pBall->setSpeed(m_pBall->getSpeed() - 1.0f);
	}
	else if (CollisionManager::leftBoundaryCheck(m_pBall)) //&& m_pBall->getHorizontalDirection() == -1)
	{
		m_pBall->setHorizontalDirection(1);
		if (m_pBall->getSpeed() >= 0.1)
			m_pBall->setSpeed(m_pBall->getSpeed() - 1.0f);
	}
	else if (CollisionManager::rightBoundaryCheck(m_pBall) && m_pBall->getHorizontalDirection() == 1)
	{
		m_pBall->setHorizontalDirection(-1);
		if (m_pBall->getSpeed() >= 0.1)
			m_pBall->setSpeed(m_pBall->getSpeed() - 1.0f);
	...
	{
		if (CollisionManager::circleSquareCombo(m_pBall, m_pBrick))
		{
			int quad = CollisionManager::bounceQuadrant(m_pBall, m_pBrick);
			glm::vec2 collisionVector = glm::vec2(m_pBrick->getPosition().x - m_pBall->getPosition().x,
				m_pBrick->getPosition().y - m_pBall->getPosition().y);

			if (collisionVector.x * collisionVector.x < collisionVector.y * collisionVector.y)
			{
				if (quad == 1 || quad == 2)		// Collision on top of the brick 
				{
					m_pBall->setVerticalDirection(-1);	// Move up
					
					if (m_pBall->getSpeed() < m_pBrick->getSpeed())
					{
						if (quad == 2)
							m_pBall->setHorizontalDirection(1);
						else
						...
						m_pBall->setAngle(-m_pBrick->getAngle());
					}
				}
				else if (quad == 3 || quad == 4)	// Collision on the bottom of the brick
				{
					m_pBall->setVerticalDirection(1);	// Move down
					//m_pBall->setAngle((m_pBrick->getAngle() + m_pBall->getAngle()) * 0.5);
			...
			else if (collisionVector.x * collisionVector.x > collisionVector.y * collisionVector.y)
			{
				if (quad == 2 || quad == 3)		// Collision on the right of the brick
				{
					m_pBall->setHorizontalDirection(1);	// Move right
				...
				else if (quad == 1 || quad == 4)	// Collision on the left side of the brick
				{
					//m_pBall->setHorizontalDirection(m_pBall->getHorizontalDirection() * -1);	// Move left
					m_pBall->setHorizontalDirection(-1);	// Move right
					...
					if (m_pBall->getSpeed() < m_pBrick->getSpeed())
					{
						...
						m_pBall->setAngle(m_pBrick->getAngle() - 180);
					}
				}
			}

			if (m_pBrick->getSpeed() >= m_pBall->getSpeed())
			{
				m_pBall->setSpeed(m_pBrick->getSpeed());
			}
			else if (m_pBrick->getSpeed() == 0)
			{
				if (m_pBall->getSpeed() >= 0.1)
					m_pBall->setSpeed(m_pBall->getSpeed() - 1.0f);
			}
			else if (m_pBrick->getSpeed() < m_pBall->getSpeed())
			{
				m_pBall->setSpeed((m_pBrick->getSpeed() + m_pBall->getSpeed()) * 0.5);
			}
		}
	}
...
{
	TheTextureManager::Instance()->load("../Assets/textures/court.png",
	...
	//m_pWall1 = new Wall();
	//m_pWall1->setWidth(32);
	//m_pWall1->setHeight(600);
	//addChild(m_pWall1);

	//m_pWall2 = new Wall();
	//m_pWall2->setWidth(800);
	//m_pWall2->setHeight(32);
	//addChild(m_pWall2);

	//m_pWall3 = new Wall();
	//m_pWall3->setWidth(800);
	//m_pWall3->setHeight(32);
	//m_pWall3->setPosition(glm::vec2(0, 600 - m_pWall3->getHeight()));
	//addChild(m_pWall3);

	//m_pWall4 = new Wall();
	//m_pWall4->setWidth(32);
	//m_pWall4->setHeight(600);
	//m_pWall4->setPosition(glm::vec2(800 - m_pWall4->getWidth(), 0));
	//addChild(m_pWall4);

	m_pBall = new Ball();
	m_pBall->setPosition(glm::vec2(400, 300));
	m_pBall->setSpeed(5);
	m_pBall->setAngle(45);
	addChild(m_pBall);
...
void StartScene::m_updateUI()
{
	...
	if (ImGui::Button("Change Scene"))
	{
		...
		Game::Instance()->changeSceneState(SceneState::SCENE2);
	}
