...
void Scene2::draw()
{
	...
	TheTextureManager::Instance()->draw("bg2", 400, 300,
...
void Scene2::update()
{
	...
	{
		m_pBullets[i]->update();
		if (CollisionManager::circleAABBCheck(m_pBullets[i], m_pShip))
		{
			// To keep the bullets staggered
			m_pBullets[i]->setPosition(glm::vec2(1 + rand() % 799, m_pBullets[i]->getPosition().y - 600));
		}
	}
...
void Scene2::start()
{
	TheTextureManager::Instance()->load("../Assets/textures/Water.png",
		"bg2", TheGame::Instance()->getRenderer());
	...
	TheSoundManager::Instance()->load("../Assets/audio/Explosion 2-SoundBible.com-1641389556.wav", "explosion", SOUND_SFX);

	for (int i = 0; i < 10; i++)
	{
		m_pBullets[i] = new Bullet();
		// This section here is to stagger the bullets so it doesn't spawn as a row of bullets
		m_pBullets[i]->setPosition(glm::vec2(m_pBullets[i]->getPosition().x, getPosition().y + 60 * i));
	}
}
